...
class DomainCheckMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if domain not in Domain.__members__.values():
            return JSONResponse(
                status_code=403, content={"message": "Invalid 'domain' value"}
            )
...
permission_exception = HTTPException(
    status_code=status.HTTP_403_FORBIDDEN, detail="Beyond permission scope"
)
...
def map_domain_to_class(tag: str, base_class):
    ...
    classes_in_user_folder = [
        file.split("_")[0] for file in files if file.endswith(".py")
    ]
